...
struct mat *subset (struct mat *m, struct vector *v, int **wrong_subset)
{
 ...
 for (i=0;i<help->row;i++) 
 {
  ...
  help=simplify(k); // FM 01.10.02 nur f�r em_zu03.dat damit 37 herauskommen
  freemat(k);
  //*->*/    printf("meta3.2_internet help r%d x c%d", help->row, help->col);   print_mat( help ); getch();
      ...
      if ((*(*(h1+u)+ii))&&(!(*(rev1+u))))
      {
        ...
        *(rev2+r2)=0; k1=*(*(h1+i)+ii); k2=*(*(h1+u)+ii);
        ...
        for (tu=0;tu<r2;tu++)
        {
         ...
         if (test1==0) break; // wenn alle r2==0 auch an der Position von tu zu finden sind, dann schmei� die neue r2 raus
        }
        if (test1==1){
